...
async function firstExisting(paths) {
  ...
  throw new Error(`必要なファイルが見つかりません: ${paths.map((item) => path.relative(root, item)).join(" / ")}`);
}
...
async function copyAssets() {
  ...
  const rootAssetPattern = /\.(?:avif|gif|ico|jpe?g|png|svg|webp)$/i;
  for (const entry of await readdir(root, { withFileTypes: true })) {
    if (entry.isFile() && rootAssetPattern.test(entry.name)) {
...
const stylesheetPath = await firstExisting([
...
console.log(`公開用の静的サイトを生成しました（NOTE ${articles.length}件）。`);
